...
class KMeansClustering:
    """
            This class shall  be used to divide the Training_Batch_Files into clusters before training.
    ...
    def elbow_plot(self,data):
        ...
        try:
            for i in range (1,11):
                ...
                kmeans.fit(data) # fitting the Training_Batch_Files to the KMeans Algorithm
            ...
            plt.ylabel('WCSS')
            #plt.show()
            plt.savefig('graphical_data/K-Means_Elbow.PNG') # saving the elbow plot locally
    ...
    def create_clusters(self,data,number_of_clusters):
        ...
        try:
            ...
            #self.Training_Batch_Files = self.Training_Batch_Files[~self.Training_Batch_Files.isin([np.nan, np.inf, -np.inf]).any(1)]
            self.y_kmeans=self.kmeans.fit_predict(data) #  divide Training_Batch_Files into clusters

            self.file_op = file_methods.File_Operations(self.file_object,self.logger_object)
            self.save_model = self.file_op.saveModel(self.kmeans, 'KMeans') # saving the KMeans model to directory
            ...
            self.logger_object.log(self.file_object,'Fitting the Training_Batch_Files to clusters failed. Exited the create_clusters method of the KMeansClustering class')
            raise Exception()